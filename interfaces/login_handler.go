...
func (au *Authenticate) Login(c *fiber.Ctx) error {
	...
	userData["id"] = u.ID
	userData["full_name"] = u.FullName
	userData["user_status"] = u.UserStatus
	userData["user_role"] = u.UserRole
...
func (au *Authenticate) Refresh(c *fiber.Ctx) error {
	...
	//is token valid?
	claimJWT := token.Claims.(jwt.MapClaims)
	tokenValid := claimJWT.Valid()
	if tokenValid != nil {
		return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
			"success": false,
			"message": "Unauthorized",
